...
![Model Performance](https://github.com/arnab-raychaudhari/ml-driven-risk-mitagation-for-underperforming-HHAs/blob/912c9c0a9bac957a012878b844512cfd84e1db18/ModelPerformance.png)

![Model Selection based on Expected Profit](https://github.com/arnab-raychaudhari/ml-driven-risk-mitagation-for-underperforming-HHAs/blob/912c9c0a9bac957a012878b844512cfd84e1db18/ModelSelectionBasedOnExpectedProfit.png)

If the Centers for Medicare & Medicaid Services (CMS) were to target the top 10%, 20%, 50%, 75%, 90%, and 100% of underperforming Home Health Agencies (HHAs), model sets 13 and 16 would yield the highest profit. The calculated Expected Profit values were multiplied by -1 to facilitate visualization in the plots. A detailed summary of the profit figures generated by the model can be reviewed by following the link provided in the appendix. The data from the Python notebook was imported into Tableau to create profit curves for each of the 27 model sets. These curves were compiled into a document and included among the artifacts uploaded as part of this project.

## RISKS AND LIMITATIONS

Throughout this process, several risks and limitations were encountered, necessitating the making of assumptions. Firstly, potential erroneous entries in the dataset posed challenges, and while data was filtered within reasonable ranges, it is possible that some errors were not detected due to the dataset’s size and limited time available. Secondly, working with only one year’s worth of data limited the ability to identify patterns and nuances across multiple years. Additionally, the lack of real-world data where CMS has actually incentivized ailing HHAs forced the assumption that those incurring losses were indeed incentivized. Moreover, in the absence of practical information on the incentive offers, the cost of the incentive was limited to administrative costs only (i.e., $1,000). Imputing missing values based on the ‘most frequent’ values and means could have introduced bias, potentially skewing the data. Although many models aimed to minimize overfitting, they remain subject to the risk of underfitting. More in-depth perspectives and methods to address the risk of underfitting would improve model reliability and predictability and enhance business decision outcomes.

## FUTURE CONSIDERATION

This analysis is currently based on one year’s worth of HHA data. Expanding the dataset to include more years would enable a more thorough understanding and representation of changing patterns in HHA data. With data spanning multiple years, predictions could be applied to future years, offering more insights into future trends and providing valuable information for healthcare experts. Additionally, interviewing industry experts would help in understanding the nuances in the data. While many assumptions were made based on research, gaining industry expertise would provide a deeper knowledge of the data and lead to more accurate assumptions for the models. Furthermore, implementing a fully automated data pipeline that continually extracts data from CMS and feeds into the ML models would enhance real-time decision making for CMS management.

## REFERENCES

Centers for Medicare & Medicaid Services Data. (n.d.).

https://data.cms.gov/

Fact sheets notice of proposed rulemaking: Medicaid and Children’s Health Insurance Program (CHIP) managed care access, finance, and Quality (CMS-2439-P). CMS.gov Centers for Medicare & Medicaid Services. (n.d.). 
